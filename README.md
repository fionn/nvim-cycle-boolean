...
When the cursor is over a word that "looks like" a boolean, we toggle it. Here "looks like" means we compare it to values like `true` and `True`. We don't consider the syntax group or tree-sitter capture to determine this, because we may want to flip booleans inside strings or comments.
